...
class TestMDAnalysisRemoval:
    ...
    def test_select_over_trajectory_raises_error(self):
        ...
        with pytest.raises(NotImplementedError, match="select_over_trajectory.*no longer supported"):
    ...
    def test_pdbqt_supplier_raises_error(self):
        ...
        with pytest.raises(NotImplementedError, match="PDBQT file support requires MDAnalysis"):
            supplier = pdbqt_supplier(["dummy.pdbqt"], template)
            next(iter(supplier))  # Trigger the error
